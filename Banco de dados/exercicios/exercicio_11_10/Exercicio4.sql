...
FROM aluguel join clientes on fkcliente = idcliente
join produto on fkproduto = idproduto
	where clientes.idcliente in (1,2,3);
    
-- Tópico 14
SELECT 
	clientes.nome as Cliente,
    produto.titulo as Produto
FROM aluguel left join clientes on fkcliente = idcliente
right join produto on fkproduto = idproduto;

-- Tópico 15
alter table aluguel drop foreign key fkAluguelCliente;
alter table aluguel drop foreign key fkAluguelProduto;
